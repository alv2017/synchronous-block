...
from api.apps.auth.tokens import decode_access_token, JWTException
...
async def get_user_me(token: str = Depends(oauth2_scheme), session=Depends(get_async_session)) -> ResponseUserModel:
    try:
        user: db_User = await decode_access_token(
            token=token,
            session=session
        )
